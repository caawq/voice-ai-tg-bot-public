...
def confirmation_text(attempt: ParseAttempt) -> str:
    ...
    if attempt.state is FlowState.failed:
        # Причина провала меняет совет. Раньше здесь был один текст на все
        # случаи, и при недоступном провайдере бот предлагал "сказать иначе" —
        # совет, который не может помочь: сколько ни переформулируй, запрос
        # всё равно не доходит. Проверено на живом боте, где Gemini отвечал
        # "User location is not supported".
        if attempt.issue is ParseIssue.provider_unavailable:
            return (
                "ИИ-сервис сейчас не отвечает, поэтому разобрать сообщение не вышло. "
                "Дело не в формулировке — попробуйте ещё раз через несколько минут."
            )
        # nothing_recognized сюда специально не попадает: этим кодом помечается
        # и пустая расшифровка, и "модель ответила, но дел не нашла" — во
        # втором случае совет переформулировать как раз работает.
        return (
